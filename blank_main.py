...
game = discord.Game("____________") # <== 1. 자신이 원하는 봇의 상태 메세지를 지정하기
bot = commands.Bot(command_prefix='>',
                   status=discord.Status.online,
                   activity=game)

TOKEN = "<디스코드 봇 토큰 입력>"  # <== 2. 자신이 발급받은 디스코드 봇의 토큰을 입력 
API_KEY = "<발급받은 API_KEY>"    # <== 3. 자신이 발급받은 교육청 API 키를 입력
...
def info_request(name):
  ...
  if SCHUL_INFO.get(name) == None:
    ...
    #################################################################
    #==== 2. URL이라는 문자열과 name이라는 문자열을 더해서 연결하기 ====#
    #==== 3. 완성한 URL을 request 모듈을 활용하여 데이터를 요청하기 ====#
    #################################################################

       # url 완성 시키기
      url = "https://open.neis.go.kr/hub/schoolInfo?SCHUL_NM="+______ 
      url += f"&Type=json&KEY={API_KEY}"  
      
      # API에서 받아온 데이터를 json형식으로 data에 저장

      data = ____________.json()       # 완성시킨 URL로 데이터를 요청하기
...
async def school(ctx, name):
  try:
    ...
    embed.add_field(name="학교 주소",value=SCHUL_INFO[name]["________"])   # <= info_request 함수에 SCHUL_INFO를
    embed.add_field(name="학교 번호",value=SCHUL_INFO[name]["________"])   # 참고해서 학교주소와 학교정보를 가져와주세요
...
async def timetable(ctx, name, grade, sclass, date):
  try:
    ...
    #################################################
    #==== 5. 문자열 포맷팅을 사용해서 변수 사용하기 ====#
    #################################################

    # 년,월,일,학년,반 에 해당하는 변수를 넣어주세요 
    # (변수명 : 년 = year, 월 = month , 일 = day, 학년=grade, 반=sclass))
    embed = discord.Embed(title=f"_____년 _____월 _____일",  
                          description=((name)+f"\n______학년______반 시간표"),
                          color=0xb9e2ee)
    ...
    #############################
    """  
    for ______________(len(data[1]['row'])):  
...
async def calendar(ctx, name, date):
  try:
    ...
    ###########################################################
    #              ==== 7. <조건문 활용하기> ====              #   
    # 문자열 형변환과 문자열 길이를 비교하는 if 비교문을 작성하기 #
    ##########################################################

    # *조건* : date의 길이가 8일때와 8이 아닐경우 
    ______________ : # date의 문자열 길이가 8이랑 같을때
